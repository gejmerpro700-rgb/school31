...
function renderDiscussionForArticle(articleId, host, lang, profile) {
    ...
    function renderList() {
        list.innerHTML = '';
        const cur = loadComments(articleId).slice().reverse();
        cur.forEach((c) => {
            ...
            if (isAdminAuthed()) {
                ...
                del.addEventListener('click', () => {
                    if (!confirm(t('dev.confirmDelete', lang) || 'Delete?')) return;
                    const all = loadAllComments();
                    all[articleId] = (all[articleId] || []).filter(x => x.id !== c.id && x.docId !== c.docId);
                    saveAllComments(all);
                    renderList();
                });
...
function openCommentsModal(articleId, lang, profile) {
    ...
    function renderList() {
        // If Firestore is active we will skip local rendering here — the onSnapshot handler will update the list.
        // However keep rendering localStorage fallback so users without Firestore still see comments.
        list.innerHTML = '';
        const fb = window.school31Firebase;
        const cur = loadComments(articleId);
        if ((!fb || !fb.db || !fb.fs || !fb.fs.onSnapshot) && cur.length === 0) {
    ...
    moreBtn.addEventListener('click', () => {
        visibleCount += 10;
        const fb = window.school31Firebase;
        if (fb && fb.db && fb.fs && fb.fs.onSnapshot && fb.fs.limit) {
            // re-subscribe with larger limit
            if (typeof unsubscribe === 'function') unsubscribe();
            subscribeSnapshot(visibleCount);
        } else {
            renderList();
        }
    });
    ...
    function subscribeSnapshot(limitCount) {
        try {
            const fb = window.school31Firebase;
            if (!(fb && fb.db && fb.fs && fb.fs.onSnapshot && fb.fs.query && fb.fs.orderBy && fb.fs.collection && fb.fs.limit)) return;

            const colRef = fb.fs.collection(fb.db, 'articles', String(articleId), 'comments');
            const q = fb.fs.query(colRef, fb.fs.orderBy('ts', 'desc'), fb.fs.limit(limitCount));
            unsubscribe = fb.fs.onSnapshot(q, (snap) => {
                const arr = [];
                snap.forEach((docSnap) => {
                    const data = docSnap.data();
                    arr.push({ id: data.id || docSnap.id, docId: docSnap.id, ts: (data.ts && data.ts.toMillis) ? data.ts.toMillis() : (data.ts || Date.now()), name: data.name || (t('discussion.anon', lang) || 'Anon'), text: data.text || '', email: data.email || '', photoURL: data.photoURL || '' });
                });

                // Update localStorage mirror so fallback and search still work
                const all = loadAllComments();
                all[articleId] = arr.slice().reverse().map((x) => ({ id: x.id, docId: x.docId, ts: x.ts, name: x.name, text: x.text, email: x.email, photoURL: x.photoURL }));
                saveAllComments(all);

                // render snapshot data (limit visibleCount)
                list.innerHTML = '';
                const slice = all[articleId].slice(-limitCount).reverse();
                slice.forEach((c) => {
                    const item = document.createElement('div');
                    item.className = 'comment';
                    ...
                    if (c.photoURL) {
                        ...
                        av.style.marginRight = '10px';
                        h.appendChild(av);
                    } else if (c.photoURL === '' && c.email === '') {
                        // try local profile avatar
                        const p = profile || loadProfile();
                        if (p && p.avatar) {
                            const av = document.createElement('img');
                            av.src = p.avatar;
                    ...
                    textEl.textContent = (c && c.text) ? c.text : '';
                    ...
                    if (isAdminAuthed()) {
                        ...
                        del.addEventListener('click', () => {
                            if (!confirm(t('dev.confirmDelete', lang) || 'Delete?')) return;
                            // if Firestore, delete doc by docId
                            if (fb && fb.fs && fb.fs.deleteDoc && c.docId) {
                                try {
                                    const dref = fb.fs.doc(fb.db, 'articles', String(articleId), 'comments', c.docId);
                                    fb.fs.deleteDoc(dref).catch(() => {
                                        // fallback: delete from local
                                        const all2 = loadAllComments();
                                        all2[articleId] = (all2[articleId] || []).filter(x => x.id !== c.id && x.docId !== c.docId);
                ...
                moreBtn.style.display = (all[articleId] && all[articleId].length > limitCount) ? 'inline-flex' : 'none';
            });
        } catch (e) {
            // ignore
        }
    ...
    // Start subscription if Firestore available
    try {
        const fb = window.school31Firebase;
        if (fb && fb.db && fb.fs && fb.fs.onSnapshot && fb.fs.query && fb.fs.orderBy && fb.fs.collection && fb.fs.limit) {
            subscribeSnapshot(visibleCount);
        }
    } catch (e) {
        // ignore
    ...
    ta.focus();
    // cleanup on close
    const origClose = closeCommentsModal;
    const cleanup = () => {
        if (typeof unsubscribe === 'function') unsubscribe();
    };
    // wrap original to include cleanup
    const _close = () => { cleanup(); origClose(); };
    closeBtn.removeEventListener('click', closeCommentsModal);
